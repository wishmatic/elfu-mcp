...
func newTestClient(t *testing.T) (*Client, string) {
	...
	base, err := url.Parse("https://elfu.example.com")
	if err != nil {
...
func upload(t *testing.T, client *Client, contentType string) (string, string) {
	...
	return url, strings.TrimPrefix(url, "https://elfu.example.com/")
}

func TestNewCreatesDirectory(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "nested", "files")

	base, _ := url.Parse("https://elfu.example.com")
...
func TestUploadAndGetRoundTrip(t *testing.T) {
	...
	if !strings.HasPrefix(url, "https://elfu.example.com/i/") {
		t.Errorf("url = %q, want https://elfu.example.com/i/ prefix", url)
	}
}
